...
#from code.code_ecore.Ecore_soft import Ecore
from code.dataloader_ecore import prepare_data_seq
...
def load_params():
    ...
    parser.add_argument("--dataset", type=str, default="./data_ecore/skep_dataset_preproc.json", help='processed EmpatheticDialogue dataset')
    ...
    ## training
    parser.add_argument("--model", type=str, default="soft", help='model name, [soft, hard]')
    ...
    parser.add_argument('--dropout', dest='dropout', type=float, default=0.2, help='dropout')#0.2
    ...
    parser.add_argument("--gen", action="store_true", help="gen")

    args = parser.parse_args()
    print_opts(args)

    args.emb_file = args.emb_file or "data_ecore/glove.6B.{}d.txt".format(str(args.emb_dim))#
    #args.emb_file = args.emb_file or "/home/ffy2020/ED/glove/glove.6B.{}d.txt".format(str(args.emb_dim))
    if (not args.test):
...
if __name__ == '__main__':
    if args.model == 'soft':
        from code.code_ecore.Ecore_soft import Ecore
        print('loading model with soft strategy')
    elif args.model == 'hard':   
        from code.code_ecore.Ecore_hard import Ecore
        print('loading model with hard strategy')
    else:
    ...
    model = Ecore(args, vocab, decoder_number=program_number)
    ...
    if args.test is False:
        try:
            ...
            for n_iter in tqdm(range(1000000)):
                ...
                if n_iter>=10000:#10000
                        args.eval_multi=True

                if (n_iter + 1) % args.check_iter == 0:#args.check_iter
        ...
        if args.specify_model:
            ...
            #checkpoint = torch.load(os.path.join(model_save_path, 'model_19999_40.6028.tar'), map_location=lambda storage, location: storage) 
            model.load_state_dict(checkpoint)
        else:
            checkpoint = torch.load(os.path.join(model_save_path, args.model+'_best.tar'),
                                    map_location=lambda storage, location: storage)
            #checkpoint = torch.load(os.path.join(model_save_path, 'model_19999_40.6028.tar'), map_location=lambda storage, location: storage) 
            weights_best = checkpoint['models']#models
            model.load_state_dict({name: weights_best[name] for name in weights_best})
        model.eval()
    ...
    if args.test or args.train_then_test:
        ...
        print("Loss\tPPL\tAccuracy", file=print_file)
        print(
        "\t{:.4f}\t{:.4f}\t{:.4f}".format( loss_test, ppl_test, acc_test), file=print_file)
