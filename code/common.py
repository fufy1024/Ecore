...
def evaluate(args, model, data, ty='valid', max_dec_step=30, print_file=None):
    ...
    outputs_emo = open(os.path.join(pred_save_path, 'multi_emo.txt'), 'w', encoding='utf-8')
    model = model.eval()
    ...
    for j, batch in pbar:   
        t1=time.time()
        loss, ppl, bce_prog, acc_prog = model.train_one_batch(batch, j, train=False,outputs_emo=outputs_emo)
        t2=time.time()
