...
class DecoderLayer1(nn.Module):
    ...
    def __init__(self, args, hidden_size, total_key_depth, total_value_depth, filter_size, num_heads,
        ...
        self.layer_norm_mha_enc1 = LayerNorm(hidden_size)
    ...
    def forward(self, inputs):
        ...
        x = self.dropout(x + y)
        x_norm = self.layer_norm_mha_enc1(x)
        #print('mask_src[:,:emotion_contexts.shape[1]]',mask_src[:,:emotion_contexts.shape[1]].shape,emotion_contexts.shape,encoder_outputs.shape,x_norm.shape)
        y, _ = self.multi_head_attention_enc_dec(x_norm,
                                                emotion_contexts,
                                                emotion_contexts,
                                                mask_src[:,:,:emotion_contexts.shape[1]],
                                                )

        
        
        
        #print('type',type(attention_weight),attention_weight)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, input_depth, total_key_depth, total_value_depth, output_depth,
        ...
        #num=2
        num=2 #4or 5
        self.emotion_output_linear = nn.Linear(num * output_depth, output_depth, bias=False)
    ...
    def forward(self, queries, keys, values, mask, vad=None, emotion_contexts=None):
        ...
        if emotion_contexts is not None:
            ...
            #print('outputs',outputs.shape, self.emotion_output_linear)
