#版本1:q种情绪间也有连接边，边为[q,q]=[q,D]*[q,D]^T
...
def subsequent_mask(size,max_len): #mask_emo:[max_len,32]
    if  size==1:
        ...
        #print('all_mask[:size,max_len:]',all_mask[:size,max_len:].shape)
    return all_mask
...
class Ecore(nn.Module):
    ...
    def train_one_batch(self, batch, iter, train=True,outputs_emo=None):
        ...
        #loss = ctx_loss +loss_emo_edge#*0.8
    ...
    def extract_emo(self,text,text_emb,emotion_score,emo,emo_emb):    
        ...
        for i in range(Bs): 
            ...
            l_i=max_l-torch.sum(text[i].data.eq(1))#有意义的token数
            ...
            if torch.sum(edge_value>0) and torch.min(edge_value)!=torch.max(edge_value):
                edge_value=edge_value/torch.max(edge_value)#最大绝对值归一化
        ...
        mask_m_all=torch.stack(mask_m_all,0)#[B,H,L,L+32],
    ...
    def emo_graph3(self,emo_words,emo_words_emb, emo_mask,emo_emb,emo_GT,length_all,emotion_context=None,emo_weight=None,edge=None,short_mask=None):
        ...
        loss_emotion = nn.CrossEntropyLoss(reduction='sum')(emotion_logit, emo_GT)#batch['emotion_label']：[bsz],即GTq情感标签,求情感预测损失

        pred_emotion = np.argmax(emotion_logit.detach().cpu().numpy(), axis=1)#[bsz]
        emotion_acc = accuracy_score(emo_GT.cpu().numpy(), pred_emotion) #情感预测准确度
        ...
        emo_logits_norm=torch.relu(emotion_soft1)#[B,32]
        emo_logits_norm=torch.softmax(emo_logits_norm,1)#[B,32]
        

     
        if len(emo_logits_norm.size())==1:
            emo_logits_norm=emo_logits_norm.unsqueeze(0)
        sort_emo=torch.sort(emo_logits_norm)[1]#[B,32],从小到达排序的ID
        ...
        edge1=torch.cat([edge[:,:,:,:-32],emo_logits_norm.unsqueeze(1).unsqueeze(1).repeat(1,self.h,edge.shape[-2],1)],-1)
